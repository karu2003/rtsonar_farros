...
def signal_process(
    Qin, Qdata, pulse_a, Nseg, Nplot, fs, maxdist, temperature, functions, stop_flag
):
    ...
    while not stop_flag.is_set():
        ...
        if found_delay and (cur_idx >= Nseg):
            ...
            Xrcv_seg = (
                abs(Xrcv[:maxsamp].copy()) / np.maximum(abs(Xrcv[0]), 1e-5)
            ) ** 0.5
...
def rtsonar(f0, f1, fs, Npulse, Nseg, Nrep, Nplot, maxdist, temperature, functions):
    ...
    t_image_update = threading.Thread(
        target=image_update, args=(Qdata, fig, Nrep, Nplot, stop_flag)
    )
